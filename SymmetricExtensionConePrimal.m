function [Cons MainCons PPTCons] = SymmetricExtensionConePrimal(rhoAB, dA, dB, k, ppt, useSym, realify)
    ...
    if usePPT
        if useSym
            for i = 1:length(ppt)
                ...
                dB1 = size(G1, 2);
                dB2 = size(G2, 2);
                conv = kron(G1, G2) \ G; % split the symmetric subspace
                tauPPT = kron(eye(dA), conv) * tau * kron(eye(dA), conv');
